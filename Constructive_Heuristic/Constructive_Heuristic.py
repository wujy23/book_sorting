from functions import get_closest,matrix_update,temp_L_update, get_value,get_score,exchange
import numpy as np
...
def get_group(L,matrix,capacity,centriod):          
        ...
        temp_L = np.array(L)
        group = np.array([centriod]) 
        
        while True: #循环读取距离中心点最近的数组，直至货架满    
            
            closest = get_closest(matrix,centriod[0]) #与当前质心相似度排名最高的点的集合
            ...
            while len(group) < capacity and np.size(closest_value,axis=0) != 0:  #当货架未满且最相近集合非空时
                ...
                group = np.vstack((group,cur))    #归入同一货架
        ...
        for b in range(np.size(dis_score,0)):
            ...
            for i in range(np.size(dis_score[a][0],0)): #i ，j 为方案的元素
                for j in range(np.size(dis_score[b][0],0)):
                    temp_a =  dis_score[a][0]
                    temp_b =  dis_score[b][0]
                    
                    temp_a , temp_b = exchange(temp_a,i,temp_b,j) 
                    
                    if get_score(temp_a,matrix) + get_score(temp_b,matrix) > dis_score[a][1] + dis_score[b][1]:
...
# matrix = np.array([[0,60,20,50],[60,0,65,40],[20,65,0,30],[50,40,30,0]])
# capacity = 2
# centriod = [0,100]
# #group,matrix,L = get_group(L,matrix,capacity,centriod)
# #plan = C_H(matrix,L,capacity)
